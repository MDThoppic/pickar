...
export default function ActiveBookingDetail({ route, navigation }) {

    const SingleCardBlock = SingleCard(SelectedBookingJSX);
    const detailScreenRedirectTo = useSelector((state) => state.quotes.detailScreenRedirectTo)
    ...
    // const selectedQuote = quotesList.quotesList.filter(quote => quote.quoteId == quoteId);

    const selectedQuote = quotesList.filter(quote => quote.quoteId == quoteId);
    const { moreInfo } = selectedQuote.length > 0 ? selectedQuote[0] : [];

    console.log(">>>>>>"+JSON.stringify(selectedQuote))
    useEffect(() => {
